    ...
    */
    unUsedSaleAttrList () {
      return this.saleAttrList.filter(
        attr => this.spuSaleAttrList.every(apuAttr => spuAttr.saleAttrName!==attr.name)
      )
    }
  },
  ...
  methods: {
    ...
    */
    ...
    handleInputConfirm (spuSaleAttr) {
      ...
      // 限制1：属性值名称必须有输入
      if (!saleAttrValename) { // 为undefined
    ...
    */
    showInput (spuSaleAttr) {
      ...
      // 让输入框自动获得焦点
      this.$nextTick(() => this.$refs.saveTageInput.focus())
    },
    ...
    */
    ...
    /*
      获取所有品牌的列表
    */
    async getTrademarkLsit() {
      const result = await this.$API.sku.getTrademarkLsit(this.spuId)
      this.spuImageList = result.data
    },
    ...
    */
    async getSaleAttrList() {
      const result = await this.$API.sku.getSaleAttrList(this.spuId)
      this.spuImageList = result.data
    },
    ...
    */
    ...
    handleRemove(file, fileList) {
      consloe.log('handleRemove', file, fileList)
      // 将已上传图片对象的列表保存起来
      this.spuImageList = fileList
    },
